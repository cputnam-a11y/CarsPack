...
package io.github.sillycarsmc.content.items;
...
public final class SillyCarsItems {
	// lowercasebtw
	public static Item BLOCKING_UPGRADE = registerItem(
		"blocking_upgrade",
		properties -> new SmithingTemplateItem(
			SillyCars.createItemDescription("smithing_template.blocking_upgrade.applies_to"),
			SillyCars.createItemDescription("smithing_template.blocking_upgrade.ingredients"),
			Component.empty(),
			Component.empty(),
			List.of(Identifier.withDefaultNamespace("container/slot/sword")),
			List.of(Identifier.withDefaultNamespace("container/slot/ingot"), SillyCars.withPath("container/slot/block")),
			properties
				.component(DataComponents.MAX_STACK_SIZE, 1)
				.component(DataComponents.RARITY, Rarity.UNCOMMON)
				.component(DataComponents.LORE, new ItemLore(List.of(SillyCars.createItemDescription("smithing_template.blocking_upgrade.description").copy().withStyle(ChatFormatting.WHITE))))
		)
	);
	...
	private static Item registerItem(final String name, final Function<Item.Properties, Item> itemFactory, final Item.Properties properties) {
		return registerItem(ResourceKey.create(Registries.ITEM, SillyCars.withPath(name)), itemFactory, properties);
	}
